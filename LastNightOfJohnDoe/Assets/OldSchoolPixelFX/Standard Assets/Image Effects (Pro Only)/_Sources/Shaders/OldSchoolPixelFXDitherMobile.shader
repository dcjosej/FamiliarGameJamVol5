Shader "Hidden/OldSchoolPixelFXDitherMobile" {
	Properties {
...
float2 parseUV(float4 c,float ditherValue)
{
	c.r += ditherValue;
	c.r = min(1,c.r);
...
float4 frag(v2f i) : SV_Target 
{
	...
	float dVal = ditherValue/(_DitherTexTotal+1);
	float4 c = tex2D(_MainTex, i.uv);
	c.rgb = tex2D(_ClutTex, parseUV(c,dVal)).rgb;
	return c;
}
...
{ 
	float2 screenParams = float2(_ScreenWidth,_ScreenHeight);
	float ditherValue = tex2D(_DitherTex, i.uv*(screenParams.xy/_DitherTexWith)).r+_Offset;
	float dVal = ditherValue*1/(_DitherTexTotal+1);
	float4 c = tex2D(_MainTex, i.uv);
